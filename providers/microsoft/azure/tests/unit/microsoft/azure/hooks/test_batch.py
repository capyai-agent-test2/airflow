...
class TestAzureBatchHook:
    ...
    def setup_test_cases(self, create_mock_connections):
        ...
        self.test_shared_key_conn_id = "test_azure_batch_shared_key"
        self.test_identity_conn_id = "test_azure_batch_identity"
        ...
        create_mock_connections(
            Connection(
                conn_id=self.test_shared_key_conn_id,
            ...
            Connection(
                conn_id=self.test_identity_conn_id,
                conn_type="azure-batch",
                extra={"account_url": self.test_account_url},
            ),
        )
    ...
    def test_uses_shared_key_auth_when_name_and_key_provided(
        ...
        mock_named_key_credential.assert_called_once_with(self.test_account_name, self.test_account_key)
    ...
    def test_fallback_to_azure_identity_credential_when_name_and_key_not_provided(
        self, mock_get_sync_default_azure_credential, mock_batch_client
    ):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_identity_conn_id)

        hook.get_conn()

        mock_get_sync_default_azure_credential.assert_called_once_with(
            managed_identity_client_id=None,
            workload_identity_tenant_id=None,
        )
        mock_batch_client.assert_called_once_with(
            endpoint=self.test_account_url,
            credential=mock_get_sync_default_azure_credential.return_value,
        )
    ...
    def test_configure_pool_rejects_cloud_service_kwargs(self):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_shared_key_conn_id)
    ...
    def test_create_pool_with_vm_config(self, mock_batch):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_shared_key_conn_id)
        mock_instance = mock_batch.return_value.create_pool
        pool = hook.configure_pool(
            pool_id="mypool",
            vm_size="test_vm_size",
            vm_node_agent_sku_id=self.test_vm_sku,
    ...
    def test_job_configuration_and_create_job(self, mock_batch):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_shared_key_conn_id)
        mock_instance = mock_batch.return_value.create_job
    ...
    def test_add_single_task_to_job(self, mock_batch):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_shared_key_conn_id)
        mock_instance = mock_batch.return_value.create_task
        ...
        mock_instance.assert_called_once_with("myjob", task)
    ...
    def test_wait_for_all_task_to_complete_all_success(self, mock_batch):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_identity_conn_id)
        hook.connection.list_tasks.return_value = iter(
            [
                batch_models.BatchTask(
                    id="mytask_1",
                    execution_info=batch_models.BatchTaskExecutionInfo(
                        retry_count=0, requeue_count=0, result=batch_models.BatchTaskExecutionResult.SUCCESS
                    ),
    ...
    def test_wait_for_all_task_to_complete_failures(self, mock_batch):
        ...
        tasks = [
            ...
            batch_models.BatchTask(
                id="mytask_2",
                execution_info=batch_models.BatchTaskExecutionInfo(
                    retry_count=0, requeue_count=0, result=batch_models.BatchTaskExecutionResult.FAILURE
                ),
                state=batch_models.BatchTaskState.COMPLETED,
            ),
        ]
        hook.connection.list_tasks.return_value = iter(tasks)
    ...
    def test_connection_failure(self, mock_batch):
        hook = AzureBatchHook(azure_batch_conn_id=self.test_identity_conn_id)
        hook.connection.list_jobs = mock.Mock(side_effect=Exception("Authentication failed."))
        status, msg = hook.test_connection()
