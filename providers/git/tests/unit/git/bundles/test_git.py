...
class TestGitDagBundle:
    ...
    def test_fetch_bare_repo_uses_full_githook_env(self, mock_githook):
        expected_env = {
            ...
            "GIT_CONFIG_COUNT": "1",
            "GIT_CONFIG_KEY_0": "http.proactiveAuth",
            "GIT_CONFIG_VALUE_0": "basic",
        }
        mock_githook.return_value.repo_url = "git@github.com:apache/airflow.git"
        mock_githook.return_value.env = expected_env

        bundle = GitDagBundle(name="my_repo", git_conn_id="git_default", tracking_ref="main")
        bundle.bare_repo = mock.MagicMock()
        custom_environment = bundle.bare_repo.git.custom_environment.return_value

        bundle._fetch_bare_repo()

        bundle.bare_repo.git.custom_environment.assert_called_once_with(**expected_env)
        custom_environment.__enter__.assert_called_once()
        custom_environment.__exit__.assert_called_once()
