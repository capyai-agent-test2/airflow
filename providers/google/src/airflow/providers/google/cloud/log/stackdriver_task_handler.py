...
def _task_instance_to_labels(ti: TaskInstance | RuntimeTI) -> dict[str, str]:
    return {
...
class StackdriverRemoteLogIO(LoggingMixin):  # noqa: D101
    ...
    def _client(self) -> gcp_logging.Client:
        ...
        return gcp_logging.Client(credentials=credentials, project=project, client_info=CLIENT_INFO)

    @cached_property
    def _logging_service_client(self) -> LoggingServiceV2Client:
        credentials, _ = self._credentials_and_project
        return LoggingServiceV2Client(credentials=credentials, client_info=CLIENT_INFO)
    ...
    def _read_single_logs_page(self, log_filter: str, page_token: str | None = None) -> tuple[str, str]:
        ...
        for entry in page.entries:
            if "message" in (entry.json_payload or {}):
                messages.append(entry.json_payload["message"])  # type: ignore[index]
...
class StackdriverTaskHandler(logging.Handler):
    ...
    @property
    def _credentials_and_project(self) -> tuple[Credentials, str]:
        return self.io._credentials_and_project

    @property
    def _client(self) -> gcp_logging.Client:
        return self.io._client

    @property
    def _logging_service_client(self) -> LoggingServiceV2Client:
        return self.io._logging_service_client

    @property
    def _transport(self) -> Transport:
        return self.io.transport
