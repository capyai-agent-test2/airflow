...
def _merge_json_with_named_parameters(json: Any, named_parameters: dict[str, Any]) -> dict[str, Any]:
    if isinstance(json, Mapping):
        return {**json, **named_parameters}
    if json:
        raise AirflowException(
            "Parameter 'json' must render to a dictionary when named parameters are provided."
        )
    return dict(named_parameters)
...
class DatabricksCreateJobsOperator(BaseOperator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.json, Mapping):
    ...
    def execute(self, context: Context) -> int:
        self.json = normalise_json_content(self._get_merged_json())
...
class DatabricksSubmitRunOperator(BaseOperator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.dbt_task is not None and self.git_source is None and "git_source" not in self.json:
            raise AirflowException("git_source is required for dbt_task")
        if (
            self.pipeline_task is not None
            and "pipeline_id" in self.pipeline_task
            and "pipeline_name" in self.pipeline_task
        ):
...
class DatabricksRunNowOperator(BaseOperator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(self.json, Mapping):
            self.json = self._get_merged_json()
        if self.job_id is not None and self.job_name is not None:
