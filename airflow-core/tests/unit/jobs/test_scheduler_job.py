...
class TestSchedulerJob:
    ...
    @pytest.mark.need_serialized_dag
    def test_create_dag_runs_for_independent_queued_asset_any_events(self, session, dag_maker):
        asset1 = Asset(uri="test://asset-any-1", name="test_asset_any_1", group="test_group")
        asset2 = Asset(uri="test://asset-any-2", name="test_asset_any_2", group="test_group")

        with dag_maker(dag_id="asset-any-producer", start_date=timezone.utcnow(), session=session):
        ...
        producer_run = dag_maker.create_dagrun(run_id="asset-any-producer-run")

        asset_ids = {
            uri: asset_id
            for uri, asset_id in session.execute(
                select(AssetModel.uri, AssetModel.id).where(AssetModel.uri.in_([asset1.uri, asset2.uri]))
            )
        }
        event1 = AssetEvent(
            asset_id=asset_ids[asset1.uri],
        ...
        with dag_maker(dag_id="asset-any-consumer", schedule=asset1 | asset2):
            pass
        ...
        assert all(dag_run.state == State.QUEUED for dag_run in created_runs)
        assert all(dag_run.creating_job_id == scheduler_job.id for dag_run in created_runs)
        assert (
            session.scalar(
                select(func.count())
                .select_from(AssetDagRunQueue)
                .where(AssetDagRunQueue.target_dag_id == consumer_dag.dag_id)
            )
            == 0
        )
    ...
    @pytest.mark.need_serialized_dag
    def test_create_dag_runs_asset_any_preserves_deferred_grouped_event_window(self, session, dag_maker):
        asset1 = Asset(
            uri="test://asset-any-deferred-grouped-1",
            name="test_asset_any_deferred_grouped_1",
            group="test_group",
        )
        asset2 = Asset(
            uri="test://asset-any-deferred-grouped-2",
            name="test_asset_any_deferred_grouped_2",
            group="test_group",
        )
        asset3 = Asset(
            uri="test://asset-any-deferred-grouped-3",
            name="test_asset_any_deferred_grouped_3",
            group="test_group",
        )

        with dag_maker(
            dag_id="asset-any-deferred-grouped-producer",
            start_date=timezone.utcnow(),
            session=session,
        ):
            BashOperator(task_id="task1", bash_command="echo 1", outlets=[asset1])
            BashOperator(task_id="task2", bash_command="echo 1", outlets=[asset2])
            BashOperator(task_id="task3", bash_command="echo 1", outlets=[asset3])
        producer_run = dag_maker.create_dagrun(run_id="asset-any-deferred-grouped-producer-run")
        ...
        with dag_maker(
            dag_id="asset-any-deferred-grouped-consumer",
        ...
        assert {event.id for event in created_runs[0].consumed_asset_events} == {events[0].id}
        assert (
            session.scalar(
                select(func.count())
                .select_from(AssetDagRunQueue)
        ...
        created_runs[0].state = State.SUCCESS
        session.flush()
        ...
        assert len(created_runs) == 2
        assert {event.id for event in created_runs[1].consumed_asset_events} == {
            events[1].id,
            events[2].id,
        }
        assert (
