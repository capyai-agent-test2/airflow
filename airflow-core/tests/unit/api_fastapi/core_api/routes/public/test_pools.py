...
class TestGetPools(TestPoolsEndpoint):
    ...
    def test_get_pools_allows_unlimited_slots(self, test_client, session):
        ...
        assert response.status_code == 200
        unlimited_pool = next(pool for pool in response.json()["pools"] if pool["name"] == "unlimited_pool")
