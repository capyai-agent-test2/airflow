...
Not all task logging configuration options are available from the ``airflow.cfg`` file. The config file
describes how to configure logging for tasks, because the logs generated by tasks are not only logged in
separate files by default but have to also be accessible via the webserver.

By default standard Airflow component logs are written to the ``$AIRFLOW_HOME/logs`` directory, but you
can also customize it by providing a custom logging configuration object. You can also create and use
logging configuration for specific operators and tasks.

Airflow routes Python logging records through ``structlog`` before writing them. Use the ``[logging]``
options in ``airflow.cfg`` to configure the final console rendering, including ``log_format``,
``log_timestamp_format``, ``callsite_parameters``, ``colored_console_log``, and ``json_logs``.

For example, to emit one JSON object per console log line for Airflow components, set:
...
Some task logging options still require that the logging config class be overwritten. You can do it by
copying the default configuration of Airflow and modifying the loggers or handlers to suit your needs.
When the logging configuration is loaded, Airflow may replace handler formatters and streams so that
records continue to be rendered consistently through ``structlog``.
...
Use a custom logging config class when you need to change Python logging handlers or logger routing, such
as assigning a different handler to a task logger. Do not use it to configure the final console log format
or JSON output; use the ``[logging]`` options described above for those settings.
