    ...
    def _evaluate_trigger_rule(
        ...
        *,
        ...
        def _get_relevant_upstream_map_indexes(upstream_id: str) -> int | range | None:
            ...
            # "depend on every upstream ti" behavior, so flexible trigger rules do
            # not evaluate it before the mapped task group has expanded. Once the
            # ti is expanded, each instance must depend on the upstream instance(s)
            # that share its map index, otherwise a single upstream result would
            # wrongly trigger every expanded instance.
            if is_mapped(task.task_group) and ti.map_index < 0:
                needs_all_upstream_tis = task.trigger_rule in (
                    TR.ONE_SUCCESS,
                    TR.ONE_FAILED,
                    TR.ONE_DONE,
                    TR.NONE_FAILED_MIN_ONE_SUCCESS,
                )
                if needs_all_upstream_tis and upstream_id not in set(
