...
class SchedulerJobRunner(BaseJobRunner, LoggingMixin):
    ...
    def _verify_integrity_if_dag_changed(self, dag_run: DagRun, session: Session) -> bool:
        ...
        if not dag_run.dag:
            return False
        # Lock TIs in PK order to match writers that update rows by primary key
        # (for example ti_update_state in the api-server). The previous bulk UPDATE
        # locked via the (dag_id, run_id) index and could deadlock with PK-ordered
        # writers during concurrent task state updates. Filtering on dag_version_id
        # ensures each batch advances past already updated rows.
        while True:
            candidates = (
                select(TI.id)
                .where(
