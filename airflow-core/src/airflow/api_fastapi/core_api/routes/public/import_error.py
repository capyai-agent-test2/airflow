...
def get_import_error(
    ...
    # did not resolve. In that case there is no per-Dag authorization scope
    # to evaluate, so return the stored stacktrace to callers who already
    # passed the import-errors view permission check on the route itself.
    if not file_dag_ids:
        return error
...
def get_import_errors(
    ...
    for import_error, file_dag_ids_iter in import_errors_result:
        ...
        # name keys did not resolve. In that case there is no per-Dag
        # authorization scope to evaluate, so return the stored stacktrace
        # to callers who already passed the import-errors view permission
        # check on the route itself.
        if not dag_ids:
